...
- Stage D.6 (in progress — backend complete, UI pending): admin UI for seeder onboarding. Shared logic extracted, Bearer-token and session-authed endpoints built. Page and Client Component pending next session. Webhook secret refresh handled separately.
