...
- **May 21** — Multi-category support across all three content tables. listings.category, affiliate_resources.category, and constellation.category all converted from TEXT to TEXT[] with ARRAY backfill. Listings and affiliate resources capped at 5 categories; constellation capped at 3. All API write paths updated (6 handlers across listings, affiliates, constellation). All submission and edit forms converted from single-select dropdown to multi-select toggle pills matching the existing practices UI pattern. Display updated on map popups, cross-seeder view, support cards, contributor cards, constellation detail. Filter dropdowns on /support and /constellation use "matches any" semantics. Search haystacks spread category arrays. Type definitions updated. Migration files created but NOT YET APPLIED to production — awaiting review and explicit go.
