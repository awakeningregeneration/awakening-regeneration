...
- **Jul 23** — Duplicate-listing soft-block shipped. Both public `/submit` and seeder `/api/seeder/place-listing` now check `normalized_name + city` against active listings before insert. If a match is found, the listing is not inserted — instead, a confirmation is shown naming the existing listing with a link to view it. The person can cancel (same business) or override (different business, proceed anyway). Public submit: `override_duplicate` flag in the request body, handled via a confirmation panel in `SubmitPageClient.tsx`. Seeder single placement: new `"duplicate"` view state in `PlacePageClient.tsx`, matching the existing `"override"` view pattern. Seeder bulk placement: surfaces as an error on the draft card. The seeder route gates both do_not_list and duplicate checks on the same `override_do_not_list` flag, so a confirmed override bypasses both on resubmission.

- **Jul 23** — Cross-seeder map view expanded to show all active listings. `/[handle]/map-view` previously filtered to `source = 'seeder_placed'` only — now shows all listings regardless of source, with attribution ("Placed by [name]" / "Community submitted"). Heading and copy updated. Accepts `?state=` query param to pre-select the state dropdown. New "See what's already in [State]" link added to the single placement form (`PlacePageClient.tsx`), visible once city and state are filled, opens the map view in a new tab.

- **Jul 22** — Steward claim confirmation email shipped. New template replaces the old Stage F-prep copy: warmer tone, submit/stories invitations ("If you know of another place that belongs on this map…" / "And if you'd ever like to share a story…"), lotus signature mark matching the `/letter` page (exported as `public/lotus-mark.png` for email-client compatibility). Sends once on claim via both the domain_match instant activation path (`app/api/steward/verify/route.ts`) and the grace-period promotion path (`app/lib/stewardshipPromotion.ts`). Duplicate-send guard: `confirmation_email_sent_at` column on `stewards` table, checked before sending and stamped immediately after. Migration: `supabase/migrations/20260722_stewards_confirmation_email_sent_at.sql` (applied to production). Retroactive send completed to the 3 already-active stewards: Rebekah Duane (Takubeh), Gathering Together Farm (Philomath, OR), Kissing Rock Coffee — all confirmed sent, timestamps set. Jill Nelson (Asana Yoga) correctly excluded — her claim was never verified, token expired; would need `/api/steward/reverify` to claim now. Script: `scripts/send-steward-confirmations-retroactive.ts` (safe to re-run, finds 0 remaining).

- **Jul 22** — Takubeh duplicate listing/steward cleanup. Deleted duplicate listing `a3caac90...` (community_submitted, wrong county, no steward) and orphaned steward row `6cb9990e...` (pending, never verified). Cascade-deleted the associated stewardship_claims row. Rebekah's real claim on listing `0b144f5a...` is untouched and active. See LOOSE_ENDS.md for full details.

- **Jul 22** — Bridge the Commons large-gift inquiry flow shipped. Form at `/founders/bridge`, API at `/api/bridge`, email notification to Ren on submit. One-time Stripe gifts capped at $499 (client + server); $500+ routed to Bridge the Commons with contextual prompt. `bridge_inquiries` table live in production with 1 existing inquiry — follow up on this separately.

- **Jul 22** — Seeder navigation fixes. Fixed five contained navigation gaps in the seeder flow: login post-submit re-entry, place-form back link, bulk-input back link, confirm dialog before bulk `placeDraft()` (warns if Email 1 will fire), and a "(required)" label on the bulk address field. Replaced NorthStarNav with a new SeederHeader component on all seeder pages. NorthStarNav previously bled through onto seeder routes and only linked to public pages — stranding seeders with no way back. NorthStarNav now hides itself on any route whose first path segment isn't in a `PUBLIC_ROUTES` allowlist (`app/components/NorthStarNav.tsx`); SeederHeader (`app/[handle]/SeederHeader.tsx`) shows a single "← Dashboard" link, wired into `app/[handle]/layout.tsx`. Note for future state rollouts: `PUBLIC_ROUTES` is a hardcoded allowlist — any new top-level public route (e.g. `/blog`) must be added there or NorthStarNav will incorrectly disappear from it too.
