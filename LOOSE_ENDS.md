...
- [ ] **Resend webhook secret refresh — pending activation on next deploy** — Fresh RESEND_WEBHOOK_SECRET generated in Resend dashboard on May 5 and saved to both .env.local and Netlify env vars. Activates with the next Netlify deploy (currently piggybacking on Stage D.6 finish). Until then, production server still verifies webhooks with the old secret — incoming bounce/delivery events return 401, but Resend retains the data and outgoing email automation is unaffected (uses RESEND_API_KEY).
